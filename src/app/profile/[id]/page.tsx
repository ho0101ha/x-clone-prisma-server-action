...
export default async function OtherProfilePage({params}: ProfilePageProps) {
  ...
  if (isNaN(targetUserId)) {
    return (
      <div className="p-8 max-w-2xl mx-auto text-red-500">
        無効なユーザーIDです。
      </div>
    );
  }
      ...
      <div className="flex justify-between items-start mb-6 border-b pb-4">
        <div>
          <h1 className="text-3xl font-bold">
            {targetUser.name || "名無しユーザー"}のプロフィール
          </h1>
        ...
        {currentUserId === null && (
          <Link
            href="/login"
            className="px-4 py-2 text-sm rounded-full bg-blue-500 text-white hover:bg-blue-600 font-bold">
            ログインしてフォロー
          </Link>
        )}
      </div>
